...
//home page
    ...
    public function superadmin_ajaxdashboard() {
        ...
        $getAllLicenses = $this->LicenseDetail->find('all', array(
            ...
            // 'conditions'=>array('LicenseDetail.status'=>1),
            //'fields'=>array('LicenseDetail.company_name','LicenseDetail.created'),
            'recursive' => 0
        ));
        if ($getAllLicenses) {
            ...
            if ($this->User->validates()) {
                ...
                if ($this->User->save($this->request->data)) {
                    ...
                    if ($this->LicenseDetail->save($this->request->data)) {
                        ...
                        $Email->subject('Reverse Advisor:Account Creation as Company License');
                        $Email->send();
            ...
            if ($this->User->validates()) {
                ...
                if ($this->User->save($this->request->data)) {
                    ...
                    if (empty($admin_id)) {
                        ...
                        $Email->subject('Reverse Advisor:Account setup as Company Admin');
                        $Email->send();
...
//                        
    ...
    public function superadmin_editRegions() {
        ...
        //get all the states
        $this->loadModel('State');
        $getAllStates = $this->State->find('all', array('fields' => array('State.id', 'State.state_code')));
            ...
            if ($this->ResidualStatesByRegions->validates()) {
                ...
                if (isset($this->request->data['ResidualStatesByRegions']['state_id'])) {
                    ...
                    foreach ($arraystates as $statekey => $stateValue) {
                        ...
                        $htmlResponse .='</td> <td>
          <div class="btn-group states_dv" data-toggle="buttons">';
    ...
    public function superadmin_getStates($region_id = null) {
        ...
        $allStates = $this->State->find('all', array('fields' => array('State.id', 'State.state_code')));

        $htmlCheckedStates = "";
        $final_k = 1;
        $htmlCheckedStates.='<div class="col-md-4 col-sm-4 col-xs-4 res_states_dv  spc">';
        ...
        foreach ($allStates as $key => $value) {
            ...
            $htmlCheckedStates .='<div  class="checkbox"><label><input type="checkbox"  value="' . $value_is . '" name="data[ResidualStatesByRegions][state_id][]" ' . $checked . ' >' . $state_code;
...
//            $getAllPlfvalue = $this->Paginate('PlfTable');
    ...
    public function superadmin_viewEmployeeDetails($company_admin_id = null) {
        $this->layout = 'ajax';
        //get the employees of the particular company admin
        echo $company_admin_id;
        $this->loadModel('EmployeeDetail');
        $getEmps = $this->EmployeeDetail->find('all', array('conditions' => array('EmployeeDetail.comp_id' => $company_admin_id, 'EmployeeDetail.status' => '1'), 'fields' => array('EmployeeDetail.emp_first_name', 'EmployeeDetail.emp_middle_name', 'EmployeeDetail.emp_last_name', 'EmployeeDetail.created', 'EmployeeDetail.status'), 'recursive' => 0));
